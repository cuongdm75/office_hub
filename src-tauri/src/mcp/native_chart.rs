        ...
        match draw_chart(chart_type, title, &data, x_key, y_key, &file_path) {
            Ok(_) => Ok(ToolCallResult {
                content: vec![ToolContent {
                    content_type: "text".to_string(),
                    text: Some(format!(
                        "Tạo biểu đồ thành công. Đã lưu tại: {}",
...
fn draw_chart(
    ...
    for item in data {
        ...
        let y_val = item
            .get(y_key)
            .and_then(|v| v.as_f64())
            .or_else(|| item.get(y_key).and_then(|v| v.as_i64()).map(|i| i as f64))
            .unwrap_or(0.0);
        if y_val > max_y {
            max_y = y_val;
        }
    ...
    let mut chart = ChartBuilder::on(&root)
        ...
        .build_cartesian_2d(-0.5f64..(parsed_data.len() as f64 - 0.5), 0.0f64..max_y)?;
    ...
    match chart_type {
        "bar" => {
            chart.draw_series(parsed_data.iter().enumerate().map(|(i, (_, y))| {
                let x0 = i as f64 - 0.3;
                let x1 = i as f64 + 0.3;
