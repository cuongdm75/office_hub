...
impl ExcelApplication {
    ...
    pub fn insert_text_at_cursor(&self, text: &str) -> anyhow::Result<String> {
        let active_cell = self.app.get_property_obj("ActiveCell")
            .map_err(|_| anyhow::anyhow!("No active Excel cell"))?;
            
        active_cell.set_property("Value", var_bstr(text))?;
        
        Ok(format!("Inserted {} characters into ActiveCell", text.len()))
    }
    ...
    pub fn replace_active_document(&self, text: &str) -> anyhow::Result<String> {
        let active_sheet = self.app.get_property_obj("ActiveSheet")
        ...
        let a1_var = cells.invoke_method("Item", vec![var_i4(1), var_i4(1)])
            .map_err(|_| anyhow::anyhow!("Could not invoke Item on Cells"))?;
        let a1 = ComObject::new(IDispatch::try_from(&a1_var)
            .map_err(|_| anyhow::anyhow!("Item is not an object"))?);
            
        a1.set_property("Value", var_bstr(text))?;

        let sheet_name = active_sheet.get_property("Name").ok()
    ...
    pub fn save_active_document(&self) -> anyhow::Result<String> {
        let active_wb = self.app.get_property_obj("ActiveWorkbook")
            .map_err(|_| anyhow::anyhow!("No active Excel workbook open"))?;

        active_wb.invoke_method("Save", vec![])?;

        let doc_name = active_wb.get_property("Name").ok()
            .and_then(|v| BSTR::try_from(&v).ok())
            .map(|b| b.to_string())
    ...
    pub fn extract_active_document(&self) -> anyhow::Result<String> {
        ...
        let used_range = active_sheet.get_property_obj("UsedRange")
        ...
        let temp_csv = format!("{}\\temp_excel_{}.csv", std::env::var("TEMP").unwrap_or_else(|_| ".".to_string()), chrono::Utc::now().format("%Y%m%d_%H%M%S"));
    ...
    #[cfg(windows)]
    pub fn add_picture(&self, image_path: &str, left: f32, top: f32, width: f32, height: f32) -> anyhow::Result<String> {
        let active_sheet = self.app.get_property_obj("ActiveSheet")
            .map_err(|_| anyhow::anyhow!("No active Excel sheet open"))?;

        let shapes = active_sheet.get_property_obj("Shapes")?;
        
        let shape_var = shapes.invoke_method("AddPicture", vec![
            var_bstr(image_path),
            var_bool(false), // LinkToFile
            var_bool(true),  // SaveWithDocument
            var_r4(left),
            var_r4(top),
            if width > 0.0 { var_r4(width) } else { var_i4(-1) }, // -1 means keep original width if needed, but COM usually prefers explicit
            if height > 0.0 { var_r4(height) } else { var_i4(-1) },
    ...
    #[cfg(not(windows))]
    pub fn add_picture(&self, _image_path: &str, _left: f32, _top: f32, _width: f32, _height: f32) -> anyhow::Result<String> {
        Ok("Excel COM Automation is only supported on Windows".to_string())
    }
}
