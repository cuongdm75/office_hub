...
async fn test_native_chart_to_ppt_pipeline() {
    ...
    let result = chart_server
        .call_tool("generate_chart_native", Some(args))
        .await
        .expect("Gọi công cụ tạo biểu đồ thất bại");
    ...
    assert!(
        parts.len() == 2,
        "Không tìm thấy đường dẫn file ảnh trong output"
    );
    ...
    match ppt_result {
        ...
        Err(e) => {
            ...
            println!(
                "Pipeline executed but PowerPoint COM failed (expected in CI): {}",
                e
            );
        }
    }
